...
enum AnnotationLegacySegmentationPipelineError: Error, LocalizedError {
...
struct AnnotationLegacySegmentationPipelineResults {
 ...
 */
class AnnotationLegacySegmentationPipeline {
    ...
    func processTransformationsRequest(imageDataHistory: [ImageData]) throws -> [CIImage] {
        if self.isProcessing {
            throw AnnotationLegacySegmentationPipelineError.isProcessingTrue
        }
        guard let homographyTransformFilter = self.homographyTransformFilter else {
            throw AnnotationLegacySegmentationPipelineError.homographyTransformFilterNil
        }
        ...
        if imageDataHistory.count <= 1 {
            ...
            throw AnnotationLegacySegmentationPipelineError.imageHistoryEmpty
        }
    ...
    func processUnionOfMasksRequest(targetValue: UInt8, bounds: DimensionBasedMaskBounds? = nil,
        ...
        guard let unionOfMasksProcessor = self.unionOfMasksProcessor else {
            throw AnnotationLegacySegmentationPipelineError.unionOfMasksProcessorNil
        }
...
//            print("Applying dimension-based mask filter")
    ...
    func processContourRequest(from ciImage: CIImage, targetValue: UInt8, isWay: Bool = false,
        ...
        guard self.contourRequestProcessor != nil else {
            throw AnnotationLegacySegmentationPipelineError.contourRequestProcessorNil
        }
