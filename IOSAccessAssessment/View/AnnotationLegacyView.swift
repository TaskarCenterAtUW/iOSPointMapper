...
enum AnnotationLegacyViewConstants {
    enum Texts {
...
struct AnnotationLegacyView: View {
    ...
    @State var options: [AnnotationOption] = AnnotationOptionFeatureClass.allCases.map { .classOption($0) }
    ...
    var body: some View {
        if (!self.isValid()) {
            ...
            orientationStack {
                HStack {
                    Spacer()
                    HostedAnnotationLegacyCameraViewController(
...
//                ScrollView(.vertical) {
                VStack {
                    HStack {
                        Spacer()
                        Text("\(AnnotationLegacyViewConstants.Texts.selectedClassPrefixText): \(Constants.SelectedAccessibilityFeatureConfig.classNames[sharedImageData.segmentedIndices[index]])")
                        Spacer()
                    }
                    
                    HStack {
                        Spacer()
                        Picker(AnnotationLegacyViewConstants.Texts.selectObjectText, selection: $annotationImageManager.selectedObjectId) {
                    ...
                    .overlay(
                        HStack {
                            ...
                            Button(action: {
                                ...
                                Image(systemName: AnnotationLegacyViewConstants.Images.infoIcon)
                                    .resizable()
...
//                                    }
                    ...
                    Button(action: {
                        ...
                        Text(index == selectedClassIndices.count - 1 ? AnnotationLegacyViewConstants.Texts.finishText : AnnotationLegacyViewConstants.Texts.nextText)
                    }
                    .padding()
                }
//                }
...
//                initializeAnnotationSegmentationPipeline()
            ...
            .alert(AnnotationLegacyViewConstants.Texts.confirmAnnotationFailedTitle, isPresented: $confirmAnnotationFailed) {
                Button(AnnotationLegacyViewConstants.Texts.confirmAnnotationFailedCancelText, role: .cancel) {
                    confirmAnnotationFailed = false
                    nextSegment()
                }
                Button(AnnotationLegacyViewConstants.Texts.confirmAnnotationFailedConfirmText) {
                    confirmAnnotationFailed = false
                    confirmAnnotationWithoutDepth()
                }
            } message: {
                Text(AnnotationLegacyViewConstants.Texts.depthOrSegmentationUnavailableText)
            }
...
//            }
            ...
            .alert(AnnotationLegacyViewConstants.Texts.uploadFailedTitle, isPresented: $isUploadFailedModalPresented) {
                Button("OK") {
    ...
    func isValid() -> Bool {
        if (self.sharedImageData.segmentedIndices.isEmpty || (index >= self.sharedImageData.segmentedIndices.count)) {
            print("Invalid index or segmentedIndices in AnnotationLegacyView")
            return false
        }
        ...
        if let selectedObjectId = annotationImageManager.selectedObjectId,
           let annotatedDetectedObjects = annotationImageManager.annotatedDetectedObjects {
            ...
            if let selectedObject = selectedObject {
                ...
                AnnotationOptionFeatureClass.allCases.map { .classOption($0) } :
                AnnotationOptionFeature.allCases.map { .individualOption($0) }
                selectedOption = selectedObject.selectedOption
            } else {
                options = AnnotationOptionFeatureClass.allCases.map { .classOption($0) }
            }
        }
    }
...
struct SelectObjectLearnMoreSheetView: View {
    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            Text(AnnotationLegacyViewConstants.Texts.selectObjectInfoLearnMoreSheetTitle)
                .font(.title)
            Text(.init(AnnotationLegacyViewConstants.Texts.selectObjectInfoLearnMoreSheetMessage))
