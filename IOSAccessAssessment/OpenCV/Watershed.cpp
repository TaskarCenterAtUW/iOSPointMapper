...
cv::Mat watershed1DMaskAndDepth (cv::Mat mask, cv::Mat depth, int labelValue) {
    ...
    // This is used by the watershed to determine the background
    // FIXME: This is a hack, we should find a better way to determine the background
...
//    // Fill labeled objects with random colors
    ...
    cv::Mat dst_final = makeBackgroundTransparent(dst, cv::Scalar(0, 0, 0, 255));
    
    return std::make_tuple(dst_final, contours, colors);
}
