...
protocol FeatureCategorical: Codable, CaseIterable, Sendable, RawRepresentable where RawValue == String {
...
struct AnyCategoricalValue: Codable, Sendable, Equatable, Hashable {
    ...
    init(typeID: String, rawValue: String) {
        self.typeID = typeID
        self.rawValue = rawValue
    }
    
    init<T: FeatureCategorical>(_ value: T) {
    ...
    func value<T: FeatureCategorical>(as type: T.Type) -> T? {
...
struct CategoricalAttributeRegistry {
    ...
    static func decode(typeID: String, raw: String) -> Any? {
    ...
    static func cases(for typeID: String) -> [AnyCategoricalValue]? {
