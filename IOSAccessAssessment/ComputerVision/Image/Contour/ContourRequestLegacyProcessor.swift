//
//  ContourRequestLegacyProcessor.swift
//  IOSAccessAssessment
//
...
enum ContourRequestLegacyProcessorError: Error, LocalizedError {
...
/**
    ContourRequestLegacyProcessor is a struct that processes contour detection requests using Vision framework.
    It performs the contour detection concurrently for each class label in the segmentation image.
 */
struct ContourRequestLegacyProcessor {
    ...
    */
    func getObjectsFromBinaryImage(
        ...
        guard let contourResults = contourRequest.results else {
            throw ContourRequestLegacyProcessorError.contourProcessingFailed
        }
...
extension ContourRequestLegacyProcessor {
    /**
